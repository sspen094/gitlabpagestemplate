...
**Status:** Draft — complete all **FILL** rows before connecting the agent.
...
|-------|------------|---------|
| **Display name** (`MFGCIDashboard`) | FILL | Acme Portal |
| **Repo slug** (`mfgcidashboard`) | FILL | `acme` |
| **Python app package** (`mfgcidashboard_app`) | FILL or N/A | `acme_app` |
| **Primary app database** (`CIDashboard`) | FILL or N/A | `AcmeDb` |
| **Next slice id** | FILL | `01` |

After filling, replace `MFGCIDashboard`, `mfgcidashboard`, `mfgcidashboard_app`, and `CIDashboard` in [`AGENTS.md`](AGENTS.md) and [`implementation-catalog.md`](implementation-catalog.md).
...
```text
You are the `agent-product-manager` agent for MFGCIDashboard.
...
- [ ] `project-connection.md` values applied; `MFGCIDashboard` placeholders updated in catalog + AGENTS
