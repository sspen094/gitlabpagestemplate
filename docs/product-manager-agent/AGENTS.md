# Ex-React — agent-product-manager instructions

**Audience:** Codex or ChatGPT agent for **slice requirements authoring**  
**Not for Cursor** — Cursor reads root [`AGENTS.md`](../../AGENTS.md) and `.cursor/skills/ex-react-core/SKILL.md`.
...
**Repo connection:** humans maintain [`project-connection.md`](project-connection.md) with host, project path, and identity (`Ex-React`, `ex-react`). Do not invent repo URLs or project ids — read them from that file or ask the human. This product targets **GitHub Pages**, not GitLab hosting.
...
6. **Project-wide documentation update** — Cursor must refresh `docs/README.md` + layer hubs + module indexes so the overall docs tree matches what shipped (feature README alone is not closeout).
